...
function! s:NeosnippetsToFzfEntry(key, val)
	return [a:key, '[[:delim:]]', a:val.action__path, '[[:delim:]]', a:val.action__line, '[[:delim:]]', printf('%s%s', (!empty(a:val.menu_abbr) ? ': ' : ''), a:val.menu_abbr)]
endfunction
...
function! s:NeosnippetsGetOptionsWithPreview(query)
	return fzf#vim#with_preview({'options': ['--delimiter', '[[:delim:]]', '--preview-window', '+{3}', '--with-nth', '{1}{4}', '--prompt', 'Snippets> ', '--query', a:query], 'placeholder': '{2}:{3}'})
endfunction
