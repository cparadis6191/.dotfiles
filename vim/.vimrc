...
" -- multiple windows --
let &statusline = ' %<%f [%{&fileencoding != "" ? &fileencoding : &encoding}] %y %{exists("g:loaded_fugitive") ? FugitiveStatusline() : ""} %{sy#repo#get_stats_decorated()} %m%r %= %-3b %-4(0x%B%) %-12(%5(%l,%)%c%V%) %P '
set hidden
